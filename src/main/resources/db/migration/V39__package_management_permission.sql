-- Bu migration rn_ icazə metadata-sına toxunur; ayrıca razılıq olmadan canlı DB-yə tətbiq edilməməlidir.
INSERT INTO public.rn_selahiyyetler(modul_id,kod,ad,aciqlama,aktiv)
SELECT id,'PAKET_IDARE_ET','Paket tərkibini idarə et','Paket xidmətlərinin tərkibini dəyişmək',true
FROM public.rn_modullar WHERE kod='APP_PACKAGES'
ON CONFLICT(kod) DO UPDATE SET modul_id=EXCLUDED.modul_id,ad=EXCLUDED.ad,aciqlama=EXCLUDED.aciqlama,aktiv=true;

INSERT INTO public.rn_route_selahiyyetleri(route,http_metod,selahiyyet_kodu,aktiv)
VALUES('/parPaket/terkib','POST','PAKET_IDARE_ET',true)
ON CONFLICT(route,http_metod) DO UPDATE SET selahiyyet_kodu=EXCLUDED.selahiyyet_kodu,aktiv=true;

INSERT INTO public.rn_rol_selahiyyetleri(rol_id,selahiyyet_id,aktiv)
SELECT r.id,s.id,true FROM public.rn_rollar r CROSS JOIN public.rn_selahiyyetler s
WHERE r.sistem_roludur AND r.aktiv AND s.kod='PAKET_IDARE_ET'
ON CONFLICT(rol_id,selahiyyet_id) DO UPDATE SET aktiv=true;
