...
public class SobeXidmetController {
    private final SobeRepository sobeRepo;
    private final XidmetRepository xidmetRepo;
    private final SobeXidmetRepository relationRepo;

    public SobeXidmetController(SobeRepository s, XidmetRepository x, SobeXidmetRepository r) {
        sobeRepo = s;
        xidmetRepo = x;
        relationRepo = r;
    }
    ...
    public String list(@RequestParam(required = false) Long sobeId, @RequestParam(required = false) Long qrupId, @RequestParam(required = false) String q, @RequestParam(defaultValue = "0") int page, @RequestParam(required = false) String bagliQ, @RequestParam(required = false) Long bagliQrupId, @RequestParam(defaultValue = "0") int bagliPage, Model m, HttpSession session) {
        ...
        if (sobeId == null && !sobeler.isEmpty()) sobeId = sobeler.getFirst().sobeId();
        Long selected = sobeId;
        if (sobeler.stream().noneMatch(s -> s.sobeId().equals(selected))) sobeId = null;
        ...
        m.addAttribute("sobeler", sobeler);
        m.addAttribute("selectedSobeId", sobeId);
        m.addAttribute("bagliXidmetler", assigned);
        m.addAttribute("movcudXidmetler", available);
        m.addAttribute("xidmetQruplari", xidmetRepo.qruplar(klinikaId).stream().filter(group -> Boolean.TRUE.equals(group.aktiv())).toList());
        ...
        m.addAttribute("bagliPage", bagliPage);
        m.addAttribute("availablePages", availablePages);
        m.addAttribute("assignedPages", assignedPages);
        return "pages/xidmetler/shobeXidmet";
    }
    ...
    private boolean belongs(Long id, HttpSession s) {
        Long kid = (Long) s.getAttribute(KlinikaController.SELECTED_KLINIKA_ID);
        return sobeRepo.findByKlinikaId(kid).stream().anyMatch(x -> x.sobeId().equals(id));
    }

    private void flash(Map<String, Object> r, RedirectAttributes a, String fallback) {
        String status = String.valueOf(r.getOrDefault("status_kodu", ""));
        String msg = String.valueOf(r.getOrDefault("mesaj", fallback));
        a.addFlashAttribute(status.toUpperCase().contains("UGUR") || status.equals("1") ? "successMessage" : "errorMessage", msg);
    }

    private String error(RedirectAttributes a, String msg, Long id) {
        a.addFlashAttribute("errorMessage", msg);
        return redirect(id);
    }

    private String redirect(Long id) {
        return "redirect:/parShobeXidmet?sobeId=" + id;
    }
}
