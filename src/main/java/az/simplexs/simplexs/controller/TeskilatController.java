...
public class TeskilatController {
    ...
    public String list(@RequestParam(required=false)String q,@RequestParam(required=false)Long tipId,
        ...
        var list=all.stream().filter(x->query.isEmpty()||contains(x.ad(),query)||contains(x.qisaAd(),query)||contains(x.vergiNomresi(),query)||contains(x.selahiyyetliSexs(),query)||contains(x.mobilNomre(),query))
        ...
        m.addAttribute("pageTitle","Təşkilatlar");m.addAttribute("activeMenuGroup","adminPanel");m.addAttribute("activeMenu","teskilatlar");m.addAttribute("teskilatlar",list);m.addAttribute("tipler",repo.tipler());m.addAttribute("umumiSayi",all.size());m.addAttribute("aktivSayi",all.stream().filter(x->Boolean.TRUE.equals(x.aktiv())).count());m.addAttribute("standartSayi",all.stream().filter(x->Boolean.TRUE.equals(x.standartdir())).count());m.addAttribute("filterApplied",!query.isEmpty()||tipId!=null||!"aktiv".equals(selectedStatus)||(standart!=null&&!standart.isBlank()));m.addAttribute("q",q);m.addAttribute("selectedTipId",tipId);m.addAttribute("selectedStatus",selectedStatus);m.addAttribute("selectedStandart",standart);return "pages/idareetme/teskilatlar";
    }
    @PostMapping("/teskilatlar/yeni") public String create(@RequestParam Long tipId,@RequestParam String ad,@RequestParam(required=false)String qisaAd,@RequestParam(required=false)String bankHesabNomresi,@RequestParam(required=false)String seherNomresi,@RequestParam(required=false)String mobilNomre,@RequestParam(required=false)String vergiNomresi,@RequestParam(required=false)String selahiyyetliSexs,@RequestParam(defaultValue="false")boolean standartdir,HttpSession s,@AuthenticationPrincipal AuthenticatedPersonal personal,RedirectAttributes a){flash(repo.yarat(klinikaId(s),tipId,ad,qisaAd,bankHesabNomresi,seherNomresi,mobilNomre,vergiNomresi,selahiyyetliSexs,standartdir,personal.personalId()),a,"Təşkilat yaradıldı.");return "redirect:/teskilatlar";}
    @PostMapping("/teskilatlar/yenile") public String update(@RequestParam Long teskilatId,@RequestParam String ad,@RequestParam(required=false)String qisaAd,@RequestParam(required=false)String bankHesabNomresi,@RequestParam(required=false)String seherNomresi,@RequestParam(required=false)String mobilNomre,@RequestParam(required=false)String vergiNomresi,@RequestParam(required=false)String selahiyyetliSexs,@RequestParam(defaultValue="false")boolean standartdir,@RequestParam(defaultValue="false")boolean aktiv,@AuthenticationPrincipal AuthenticatedPersonal personal,RedirectAttributes a){flash(repo.yenile(teskilatId,ad,qisaAd,bankHesabNomresi,seherNomresi,mobilNomre,vergiNomresi,selahiyyetliSexs,standartdir,aktiv,personal.personalId()),a,"Təşkilat yeniləndi.");return "redirect:/teskilatlar";}
