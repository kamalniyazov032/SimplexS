...
public class AnbarController {
    ...
    public AnbarController(AnbarRepository repository, MessageSource messages) {
    ...
    public String page(@PathVariable String tab, @RequestParam(defaultValue = "aktiv") String status,
        ...
        Boolean aktiv = status(status);
        model.addAttribute("pageTitle", msg("warehouse.tab." + tab));
        model.addAttribute("activeMenu", "anbar." + tab);
        ...
        model.addAttribute("qruplar", java.util.List.of());
        model.addAttribute("materiallar", java.util.List.of());
        model.addAttribute("kateqoriyalar", java.util.List.of());
        model.addAttribute("emeliyyatlar", java.util.List.of());
        ...
        switch (tab) {
            ...
            case "emeliyyatlar" -> {
                model.addAttribute("kateqoriyalar", repository.kateqoriyalar(istiqamet));
                model.addAttribute("emeliyyatlar", repository.emeliyyatlar(k, istiqamet, aktiv).stream().filter(x -> matches(axtaris, x.ad(), x.kateqoriyaAdi(), x.aciqlama())).toList());
            }
            default -> {
                model.addAttribute("anbarNovleri", repository.anbarNovleri());
                model.addAttribute("anbarlar", repository.anbarlar(k, null, aktiv, axtaris));
            }
        }
    ...
    public String qrup(@RequestParam Map<String, String> f, HttpSession s, @AuthenticationPrincipal AuthenticatedPersonal p, RedirectAttributes a) {
        Long id = L(f, "id");
        flash(repository.qrupYaddaSaxla(id == null, clinic(s), id, L(f, "qrupNovuId"), f.get("ad"), f.get("aciqlama"), B(f, "xesteyeCixis"), B(f, "sifarisdeGorunsun"), B(f, "xesteMaterial"), B(f, "cihazaCixis"), B(f, "aktiv"), p.personalId()), a);
        return back("qruplar");
    }
    ...
    public String emeliyyat(@RequestParam Map<String, String> f, HttpSession s, @AuthenticationPrincipal AuthenticatedPersonal p, RedirectAttributes a) {
        Long id = L(f, "id");
        flash(repository.emeliyyatYaddaSaxla(id == null, id, clinic(s), L(f, "kateqoriyaId"), f.get("ad"), B(f, "standartdir"), f.get("aciqlama"), B(f, "aktiv"), p.personalId()), a);
        return back("emeliyyatlar");
    }

    @PostMapping("/parametrler/anbar/anbar")
    public String anbar(@RequestParam Map<String, String> f, HttpSession s, @AuthenticationPrincipal AuthenticatedPersonal p, RedirectAttributes a) {
        Long id = L(f, "id");
        flash(repository.anbarYaddaSaxla(id == null, id, clinic(s), L(f, "anbarNovuId"), f.get("ad"), f.get("aciqlama"), B(f, "telebAnbaridir"), B(f, "telebdeStokGorunsun"), B(f, "dermanPaketi"), B(f, "istehsalCixisi"), B(f, "mehvCixisi"), B(f, "aktiv"), f.get("baslamaTarixi") == null || f.get("baslamaTarixi").isBlank() ? null : java.time.LocalDate.parse(f.get("baslamaTarixi")), f.get("cixisUsulu"), p.personalId()), a);
        return back("anbarlar");
    }
    ...
    private Boolean status(String s) {
        return "hamisi".equals(s) ? null : !"passiv".equals(s);
    }

    private String back(String tab) {
        return "redirect:/anbar/" + tab;
    }

    private void flash(Map<String, Object> r, RedirectAttributes a) {
        String code = String.valueOf(r.getOrDefault("status_kodu", ""));
        boolean ok = code.equals("1") || code.toUpperCase().contains("UGUR");
        a.addFlashAttribute(ok ? "successMessage" : "errorMessage", String.valueOf(r.getOrDefault("mesaj", msg(ok ? "warehouse.saved" : "warehouse.save_failed"))));
    }
    ...
    private static boolean matches(String query, String... values) {
        ...
        return java.util.Arrays.stream(values).filter(java.util.Objects::nonNull).anyMatch(v -> v.toLowerCase(java.util.Locale.ROOT).contains(q));
    }

    private static boolean B(Map<String, String> f, String k) {
        return "true".equals(f.get(k));
    }

    private static Long L(Map<String, String> f, String k) {
        try {
            return f.get(k) == null || f.get(k).isBlank() ? null : Long.valueOf(f.get(k));
        } catch (Exception e) {
            return null;
        }
    }

    private static BigDecimal D(Map<String, String> f, String k) {
        try {
            return f.get(k) == null || f.get(k).isBlank() ? null : new BigDecimal(f.get(k));
        } catch (Exception e) {
            return null;
        }
    }
