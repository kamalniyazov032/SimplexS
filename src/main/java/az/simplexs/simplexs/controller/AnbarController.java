...
public class AnbarController {
    ...
    public String page(@PathVariable String tab,@RequestParam(defaultValue="aktiv")String status,
        ...
        switch(tab){
            case "firmalar" -> model.addAttribute("firmalar",repository.firmalar(aktiv).stream().filter(x->matches(axtaris,x.ad(),x.unvan(),x.email())).toList());
            ...
            case "qruplar" -> {model.addAttribute("qrupNovleri",repository.qrupNovleri());model.addAttribute("qruplar",repository.qruplar(k,aktiv).stream().filter(x->matches(axtaris,x.ad(),x.aciqlama())).toList());}
            ...
            case "emeliyyatlar" -> {model.addAttribute("kateqoriyalar",repository.kateqoriyalar(istiqamet));model.addAttribute("emeliyyatlar",repository.emeliyyatlar(k,istiqamet,aktiv).stream().filter(x->matches(axtaris,x.ad(),x.kateqoriyaAdi(),x.aciqlama())).toList());}
    ...
    public String firma(@RequestParam Map<String,String> f,HttpSession s,@AuthenticationPrincipal AuthenticatedPersonal p,RedirectAttributes a){Long id=L(f,"id");flash(id==null?repository.firmaYarat(clinic(s),f.get("ad"),f.get("unvan"),f.get("telefon"),f.get("faks"),f.get("email"),f.get("qeyd"),f.get("bankAdi"),f.get("bankHesabi"),f.get("vergiNomresi"),f.get("vergiIdaresi"),p.personalId()):repository.firmaYenile(clinic(s),id,f.get("ad"),f.get("unvan"),f.get("telefon"),f.get("faks"),f.get("email"),f.get("qeyd"),f.get("bankAdi"),f.get("bankHesabi"),f.get("vergiNomresi"),f.get("vergiIdaresi"),B(f,"aktiv"),p.personalId()),a);return back("firmalar");}
