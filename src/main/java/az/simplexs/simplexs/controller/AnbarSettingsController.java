...
public class AnbarSettingsController {
    ...
    private void year(int year) {
        if (year < 1900 || year > 9999) throw new ResponseStatusException(HttpStatus.BAD_REQUEST);
    }
    ...
    private void warehouse(Long k, Long id) {
        if (catalog.anbarlar(k, null, null, null).stream().noneMatch(x -> id.equals(x.id())))
    ...
    public String warehouseSave(@PathVariable Long id, @PathVariable String section, @RequestParam Map<String, String> form,
        ...
        Long k = clinic(session);
        warehouse(k, id);
        int y = Integer.parseInt(form.getOrDefault("il", String.valueOf(Year.now().getValue())));
        year(y);
    ...
    public String bulk(@RequestParam int il, @RequestParam int ay, @RequestParam(defaultValue = "false") boolean kilidlidir,
                       HttpSession session, @AuthenticationPrincipal AuthenticatedPersonal actor, RedirectAttributes attrs) {
        year(il);
        if (ay < 1 || ay > 12) throw new ResponseStatusException(HttpStatus.BAD_REQUEST);
    ...
    public String unitsPage(@PathVariable Long id, HttpSession session, Model model) {
        ...
        var material = catalog.materiallar(k, null, null, null)
                .stream()
                .filter(x -> id.equals(x.id()))
                .findFirst()
                .orElseThrow(() -> new ResponseStatusException(HttpStatus.NOT_FOUND));

        var existing = settings.units(id);

        Map<Long, Map<String, Object>> byId = new HashMap<>();
        for (var row : existing)
            byId.put(((Number) row.get("vahid_id")).longValue(), row);

        model.addAttribute("pageTitle", material.ad());
        model.addAttribute("activeMenu", "anbar.materiallar");
        model.addAttribute("id", id);
        model.addAttribute("units", catalog.vahidler(k, true, null));
        model.addAttribute("existing", byId);
    ...
    public String saveUnits(@PathVariable Long id, @RequestParam Map<String, String> form, HttpSession session,
        ...
        Long k = clinic(session);
        if (catalog.materiallar(k, null, null, null).stream().noneMatch(x -> id.equals(x.id())))
            throw new ResponseStatusException(HttpStatus.NOT_FOUND);
        List<Map<String, Object>> rows = new ArrayList<>();
        Map<Long, Map<String, Object>> locked = new HashMap<>();
        for (var row : settings.units(id))
            if (Boolean.TRUE.equals(row.get("ana_vahiddir")) || !Boolean.TRUE.equals(row.get("deyisdirile_biler")))
                locked.put(((Number) row.get("vahid_id")).longValue(), row);
        for (var unit : catalog.vahidler(k, true, null)) {
            ...
            if (current != null) {
                if (!Boolean.TRUE.equals(current.get("ana_vahiddir")) && Boolean.TRUE.equals(current.get("aktiv"))) {
                    ...
                    preserved.put("vahid_id", unit.id());
                    preserved.put("ana_vahide_emsal", current.get("ana_vahide_emsal"));
                    preserved.put("sira_no", current.get("sira_no"));
                    rows.add(preserved);
                }
                ...
                row.put("vahid_id", unit.id());
                row.put("ana_vahide_emsal", factor);

                String order = form.get("order_" + unit.id());
                row.put("sira_no", order == null || order.isBlank() ? null : Integer.valueOf(order));
