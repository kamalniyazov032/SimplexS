...
public class TercumeRepository {
    ...
    public TercumeRepository(NamedParameterJdbcTemplate jdbc) {
        this.jdbc = jdbc;
    }
    ...
    public List<Dil> diller() {
        ...
        try {
            return jdbc.query("SELECT id,kod,ad,yerli_ad,standartdir,aktiv,sira_no FROM public.kn_diller ORDER BY sira_no,ad", Map.of(),
            ...
            return List.of(new Dil(null, "az", "Azərbaycan dili", "Azərbaycan dili", true, true, 1));
        }
    }
    ...
    public Map<String, String> tercumeler(String dilKodu) {
        ...
        try {
            return jdbc.query("SELECT t.acar,t.deyer FROM public.kn_interfeys_tercumeleri t JOIN public.kn_diller d ON d.id=t.dil_id WHERE d.kod=:kod AND d.aktiv", Map.of("kod", dilKodu),
                    r -> {
                        var result = new java.util.LinkedHashMap<String, String>();
                        while (r.next()) result.put(r.getString(1), r.getString(2));
                        return result;
                    });
        } catch (DataAccessException e) {
            schemaAvailable = false;
    ...
    public void dilYarat(String kod, String ad, String yerliAd) {
        jdbc.update("INSERT INTO public.kn_diller(kod,ad,yerli_ad,standartdir,aktiv,sira_no) VALUES(lower(:kod),:ad,:yerli,false,true,(SELECT COALESCE(max(sira_no),0)+1 FROM public.kn_diller))", new MapSqlParameterSource().addValue("kod", kod.trim()).addValue("ad", ad.trim()).addValue("yerli", yerliAd.trim()));
        version.incrementAndGet();
    }

    public void dilYenile(Long id, String ad, String yerliAd, boolean aktiv) {
        jdbc.update("UPDATE public.kn_diller SET ad=:ad,yerli_ad=:yerli,aktiv=:aktiv,yenilenme_tarixi=now() WHERE id=:id AND NOT standartdir", new MapSqlParameterSource().addValue("id", id).addValue("ad", ad.trim()).addValue("yerli", yerliAd.trim()).addValue("aktiv", aktiv));
        version.incrementAndGet();
    }

    public void tercumeYaddaSaxla(String dilKodu, String acar, String deyer) {
        jdbc.update("""
                ...
                """, new MapSqlParameterSource().addValue("kod", dilKodu).addValue("acar", acar).addValue("deyer", deyer.trim()));
        version.incrementAndGet();
    }

    public List<MelumatNovu> melumatNovleri() {
        return List.of(
    ...
    public List<MelumatTercumeSetri> melumatSetirleri(String nov) {
        String sql = MELUMAT_SQL.get(nov);
        if (sql == null) throw new IllegalArgumentException("Naməlum məlumat növü");
        var translations = jdbc.query("SELECT t.menbe_id,t.saha,d.kod,t.deyer FROM public.kn_melumat_tercumeleri t JOIN public.kn_diller d ON d.id=t.dil_id WHERE t.melumat_novu=:nov", Map.of("nov", nov), r -> {
            var x = new java.util.HashMap<String, String>();
            while (r.next()) x.put(r.getLong(1) + "|" + r.getString(2) + "|" + r.getString(3), r.getString(4));
            return x;
        });
        return jdbc.query("SELECT * FROM (" + sql + ") x ORDER BY ad", Map.of(), (r, n) -> {
            Long id = r.getLong("id");
            String saha = r.getString("saha");
            var values = new java.util.LinkedHashMap<String, String>();
            diller().stream().filter(d -> !Boolean.TRUE.equals(d.standartdir())).forEach(d -> values.put(d.kod(), translations.getOrDefault(id + "|" + saha + "|" + d.kod(), "")));
            return new MelumatTercumeSetri(id, r.getString("kod"), r.getString("ad"), saha, values);
        });
    }

    public void melumatTercumesiYaddaSaxla(String nov, Long id, String saha, String dilKodu, String deyer) {
        if (!MELUMAT_SQL.containsKey(nov) || !"ad".equals(saha))
            throw new IllegalArgumentException("Yanlış məlumat növü və ya sahə");
        jdbc.update("""
