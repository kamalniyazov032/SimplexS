...
public class AnbarRepository {
    ...
    public Map<String, Object> firmaYenile(Long id, String ad, String unvan, String telefon, String faks, String email, String qeyd, String bank, String hesab, String vergi, String idare, boolean aktiv, Long personal) {
        return one("SELECT * FROM public.fn_firma_yenile(p_firma_id=>CAST(:id AS bigint),p_ad=>:ad,p_unvan=>:u,p_unvan_deyisdirilsin=>true,p_is_telefonu=>:t,p_is_telefonu_deyisdirilsin=>true,p_faks_nomresi=>:f,p_faks_nomresi_deyisdirilsin=>true,p_email=>:e,p_email_deyisdirilsin=>true,p_qeyd=>:q,p_qeyd_deyisdirilsin=>true,p_bank_adi=>:b,p_bank_adi_deyisdirilsin=>true,p_bank_hesab_nomresi=>:h,p_bank_hesab_nomresi_deyisdirilsin=>true,p_vergi_nomresi=>:v,p_vergi_nomresi_deyisdirilsin=>true,p_vergi_idaresi=>:i,p_vergi_idaresi_deyisdirilsin=>true,p_aktiv=>:a,p_yenileyen_personal_id=>CAST(:p AS bigint))", p().addValue("id", id).addValue("ad", n(ad)).addValue("u", n(unvan)).addValue("t", n(telefon)).addValue("f", n(faks)).addValue("e", n(email)).addValue("q", n(qeyd)).addValue("b", n(bank)).addValue("h", n(hesab)).addValue("v", n(vergi)).addValue("i", n(idare)).addValue("a", aktiv).addValue("p", personal));
    }
    ...
    public Map<String, Object> vahidYenile(Long id, String ad, Long alt, java.math.BigDecimal emsal, boolean sifaris, boolean aktiv, Long personal) {
        return one("SELECT * FROM public.fn_vahid_yenile(p_vahid_id=>CAST(:id AS bigint),p_ad=>:ad,p_alt_vahid_id=>CAST(:alt AS bigint),p_alt_vahid_deyisdirilsin=>true,p_vurma_emsali=>CAST(:em AS numeric),p_vurma_emsali_deyisdirilsin=>true,p_sifarisde_gorunsun=>:s,p_aktiv=>:a,p_yenileyen_personal_id=>CAST(:p AS bigint))", p().addValue("id", id).addValue("ad", ad.trim()).addValue("alt", alt).addValue("em", emsal).addValue("s", sifaris).addValue("a", aktiv).addValue("p", personal));
    }
    ...
    public List<MehsulQrupu> qruplar(Long k, Boolean aktiv, Long qrupNovuId) {
        return jdbc.query("SELECT * FROM public.fn_mehsul_qrup_siyahisi(p_klinika_id=>CAST(:k AS bigint),p_qrup_novu_id=>CAST(:nov AS bigint),p_aktiv=>CAST(:a AS boolean)) ORDER BY sira_no NULLS LAST,ad", p().addValue("k", k).addValue("a", aktiv).addValue("nov", qrupNovuId), (r, i) -> new MehsulQrupu(l(r, "mehsul_qrupu_id"), l(r, "qrup_novu_id"), s(r, "qrup_novu_kodu"), s(r, "qrup_novu_adi"), s(r, "ad"), s(r, "aciqlama"), b(r, "xesteye_cixis_edile_biler"), b(r, "sifarisde_gorunsun"), b(r, "xeste_material_cixisinda_gorunsun"), b(r, "cihaza_cixis_edile_biler"), b(r, "aktiv")));
    }
    ...
    public Map<String, Object> qrupYaddaSaxla(boolean yeni, Long k, Long id, Long nov, String ad, String ac, boolean xeste, boolean sifaris, boolean material, boolean cihaz, boolean aktiv, Long personal) {
        String sql = yeni ? "SELECT * FROM public.fn_mehsul_qrupu_yarat(p_klinika_id=>CAST(:k AS bigint),p_qrup_novu_id=>CAST(:n AS bigint),p_ad=>:ad,p_aciqlama=>:ac,p_xesteye_cixis_edile_biler=>:x,p_sifarisde_gorunsun=>:s,p_xeste_material_cixisinda_gorunsun=>:m,p_cihaza_cixis_edile_biler=>:c,p_yaradan_personal_id=>CAST(:p AS bigint))" : "SELECT * FROM public.fn_mehsul_qrupu_yenile(p_mehsul_qrupu_id=>CAST(:id AS bigint),p_qrup_novu_id=>CAST(:n AS bigint),p_ad=>:ad,p_aciqlama=>:ac,p_aciqlama_deyisdirilsin=>true,p_xesteye_cixis_edile_biler=>:x,p_sifarisde_gorunsun=>:s,p_xeste_material_cixisinda_gorunsun=>:m,p_cihaza_cixis_edile_biler=>:c,p_aktiv=>:a,p_yenileyen_personal_id=>CAST(:p AS bigint))";
    ...
    public List<Material> materiallar(Long k, Long qrup, Boolean aktiv, String axtaris) {
        return jdbc.query("SELECT * FROM public.fn_material_siyahisi(p_klinika_id=>CAST(:k AS bigint),p_qrup_id=>CAST(:q AS bigint),p_aktiv=>CAST(:a AS boolean)) WHERE (CAST(:x AS varchar) IS NULL OR ad ILIKE '%'||:x||'%' OR qisa_ad ILIKE '%'||:x||'%' OR barkod_nomresi ILIKE '%'||:x||'%') ORDER BY sira_no NULLS LAST,ad", p().addValue("k", k).addValue("q", qrup).addValue("a", aktiv).addValue("x", n(axtaris)), (r, i) -> new Material(l(r, "material_id"), l(r, "qrup_id"), s(r, "qrup_adi"), l(r, "ana_vahid_id"), s(r, "ana_vahid_adi"), s(r, "ad"), s(r, "qisa_ad"), s(r, "barkod_nomresi"), d(r, "minimum_miqdar"), d(r, "maksimum_miqdar"), b(r, "mehv_edile_biler"), b(r, "paketden_kenar"), b(r, "aktiv"), s(r, "farmasevtik_melumat"), s(r, "istifade_qaydasi"), d(r, "minimum_doza"), d(r, "maksimum_doza"), in(r, "xeste_ucun_maksimum_teleb_muddeti"), d(r, "xeste_ucun_maksimum_teleb_miqdari"), in(r, "yas_asagi_heddi"), in(r, "yas_yuxari_heddi"), d(r, "beden_cekisi"), b(r, "avtomatik_hekim_tesdiqi_olmasin"), b(r, "vahid_deyisdirile_biler")));
    }
    ...
    public List<AnbarNovu> anbarNovleri() {
        return jdbc.query("SELECT * FROM public.fn_anbar_novu_siyahisi() ORDER BY sira_no NULLS LAST,ad", (r, i) -> new AnbarNovu(l(r, "anbar_novu_id"), s(r, "ad"), s(r, "aciqlama"), in(r, "sira_no"), b(r, "aktiv")));
    }

    public List<Anbar> anbarlar(Long k, Long nov, Boolean aktiv, String axtaris) {
        return jdbc.query("SELECT * FROM public.fn_anbar_siyahisi(p_klinika_id=>CAST(:k AS bigint),p_anbar_novu_id=>CAST(:n AS bigint),p_aktiv=>CAST(:a AS boolean)) WHERE (CAST(:x AS varchar) IS NULL OR ad ILIKE '%'||:x||'%') ORDER BY sira_no NULLS LAST,ad", p().addValue("k", k).addValue("n", nov).addValue("a", aktiv).addValue("x", n(axtaris)), (r, i) -> new Anbar(l(r, "anbar_id"), l(r, "anbar_novu_id"), s(r, "anbar_novu_adi"), s(r, "ad"), s(r, "aciqlama"), b(r, "teleb_anbaridir"), b(r, "telebde_stok_gorunsun"), b(r, "derman_paketi_tetbiq_edilsin"), b(r, "istehsal_cixisi_edile_biler"), b(r, "mehv_cixisi_edile_biler"), in(r, "sira_no"), b(r, "aktiv"), r.getObject("istifadeye_baslama_tarixi", java.time.LocalDate.class), s(r, "cixis_usulu"), s(r, "effektiv_cixis_usulu"), dt(r, "yaranma_tarixi")));
    }
