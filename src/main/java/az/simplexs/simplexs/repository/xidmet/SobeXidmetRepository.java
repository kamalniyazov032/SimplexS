...
public class SobeXidmetRepository {
    ...
    public List<SobeXidmeti> findPage(Long sobeId,String query,int limit,int offset){if(sobeId==null)return List.of();return jdbc.query("SELECT * FROM public.fn_xidmet_sobe_siyahisi(?,true) f WHERE (CAST(? AS text) IS NULL OR f.xidmet_adi ILIKE '%'||CAST(? AS text)||'%' OR f.xidmet_kodu ILIKE '%'||CAST(? AS text)||'%') ORDER BY xidmet_qrupu_adi,xidmet_adi LIMIT ? OFFSET ?",ps->{ps.setLong(1,sobeId);ps.setString(2,blank(query));ps.setString(3,blank(query));ps.setString(4,blank(query));ps.setInt(5,limit);ps.setInt(6,offset);},this::map);}
    public long count(Long sobeId,String query){if(sobeId==null)return 0;Long n=jdbc.queryForObject("SELECT count(*) FROM public.fn_xidmet_sobe_siyahisi(?,true) f WHERE (CAST(? AS text) IS NULL OR f.xidmet_adi ILIKE '%'||CAST(? AS text)||'%' OR f.xidmet_kodu ILIKE '%'||CAST(? AS text)||'%')",Long.class,sobeId,blank(query),blank(query),blank(query));return n==null?0:n;}
