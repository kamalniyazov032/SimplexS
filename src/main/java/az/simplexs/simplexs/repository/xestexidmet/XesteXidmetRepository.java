...
public class XesteXidmetRepository {
    ...
    public Map<String, Object> hazirla(Long gelisId, Map<String, Object> row, Long istekId, Long personalId) {
        boolean rutin = row.get("rutin_id") != null;
        String sql = "SELECT * FROM public." + (rutin ? "fn_rutin_xidmetini_hazirla(p_rutin_id => CAST(:rutin_id AS bigint), " : "fn_xeste_xidmetini_hazirla(") + "p_gelis_id => CAST(:gelis AS bigint), p_xidmet_id => CAST(:xidmet_id AS bigint), p_sobe_id => CAST(:sobe_id AS bigint), p_xidmet_tarixi => CAST(:xidmet_tarixi AS date), p_gonderen_hekim_id => CAST(:gonderen_hekim_id AS bigint), p_isteyen_hekim_id => CAST(:isteyen_hekim_id AS bigint), p_icra_eden_hekim_id => CAST(:icra_eden_hekim_id AS bigint), p_miqdar => CAST(:miqdar AS numeric), p_tecili => CAST(:tecili AS boolean), p_istek_id => CAST(:istek AS bigint), p_emel_eden_personal_id => CAST(:personal AS bigint))";
        return jdbc.queryForMap(sql, new MapSqlParameterSource(row).addValue("gelis", gelisId).addValue("istek", istekId).addValue("personal", personalId));
    }
