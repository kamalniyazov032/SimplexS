...
public class MuhasibatKoduRepository {
    ...
    public List<MuhasibatKodu> siyahi(Long klinikaId,Boolean aktiv){return jdbc.query("SELECT * FROM public.fn_muhasibat_kodu_siyahisi(CAST(:klinika AS bigint),CAST(:aktiv AS boolean)) ORDER BY tip_adi,ad",new MapSqlParameterSource("aktiv",aktiv).addValue("klinika",klinikaId),(r,n)->new MuhasibatKodu(l(r,"muhasibat_kodu_id"),l(r,"tip_id"),r.getString("tip_kodu"),r.getString("tip_adi"),r.getString("ad"),r.getString("aciqlama"),r.getObject("aktiv",Boolean.class)));}
    ...
    public Map<String,Object> yenile(Long id,String ad,String aciqlama,boolean aktiv){return one("SELECT * FROM public.fn_muhasibat_kodu_yenile(p_muhasibat_kodu_id=>:id,p_ad=>:ad,p_aciqlama=>:aciqlama,p_aciqlama_deyisdirilsin=>true,p_aktiv=>:aktiv)",new MapSqlParameterSource().addValue("id",id).addValue("ad",ad.trim()).addValue("aciqlama",blank(aciqlama)).addValue("aktiv",aktiv));}
