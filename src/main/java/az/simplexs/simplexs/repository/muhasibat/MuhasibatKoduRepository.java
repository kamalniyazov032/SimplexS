...
public class MuhasibatKoduRepository {
    ...
    public List<MuhasibatKodu> siyahi(){return jdbc.query("SELECT * FROM public.fn_muhasibat_kodu_siyahisi(CAST(:aktiv AS boolean)) ORDER BY tip_adi,ad",new MapSqlParameterSource("aktiv",null),(r,n)->new MuhasibatKodu(l(r,"muhasibat_kodu_id"),r.getString("tip_kodu"),r.getString("tip_adi"),r.getString("ad"),r.getString("aciqlama"),r.getObject("aktiv",Boolean.class)));}
    public Map<String,Object> yarat(Long tipId,String ad){return one("SELECT * FROM public.fn_muhasibat_kodu_yarat(p_tip_id=>:tip,p_ad=>:ad)",new MapSqlParameterSource().addValue("tip",tipId).addValue("ad",ad.trim()));}
