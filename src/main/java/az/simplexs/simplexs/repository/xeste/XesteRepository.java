...
public class XesteRepository {
    ...
    public List<Xeste> siyahi(Long klinika, Boolean aktiv, String q, Long teskilatId, Long cursor, int limit) {
        if (blank(q) == null) {
            return jdbc.query("SELECT * FROM public.fn_xeste_siyahisi_sehifeli(p_klinika_id=>CAST(:k AS bigint),p_aktiv=>CAST(:a AS boolean),p_teskilat_id=>CAST(:t AS bigint),p_son_xeste_id=>CAST(:cursor AS bigint),p_limit=>:limit)", new MapSqlParameterSource("k", klinika).addValue("a", aktiv).addValue("t", teskilatId).addValue("cursor", cursor).addValue("limit", limit), this::map);
        }
        return jdbc.query("SELECT * FROM public.fn_xeste_siyahisi(p_klinika_id=>CAST(:k AS bigint),p_aktiv=>CAST(:a AS boolean)) WHERE (CAST(:q AS varchar) IS NULL OR lower(coalesce(xeste_kodu, '') || ' ' || coalesce(fin_kodu, '') || ' ' || coalesce(sexsiyyet_vesiqesi_nomresi, '') || ' ' || coalesce(ad, '') || ' ' || coalesce(soyad, '') || ' ' || coalesce(ata_adi, '') || ' ' || coalesce(mobil_nomre, '') || ' ' || coalesce(ikinci_mobil_nomre, '') || ' ' || coalesce(sosial_kart_nomresi, '') || ' ' || coalesce(is_yeri, '') || ' ' || coalesce(vezifesi, '') || ' ' || coalesce(pesesi, '')) LIKE '%' || lower(CAST(:q AS varchar)) || '%') AND (CAST(:t AS bigint) IS NULL OR default_teskilat_id=:t) AND (CAST(:cursor AS bigint) IS NULL OR xeste_id<:cursor) ORDER BY xeste_id DESC LIMIT :limit", new MapSqlParameterSource("k", klinika).addValue("a", aktiv).addValue("q", blank(q)).addValue("t", teskilatId).addValue("cursor", cursor).addValue("limit", limit), this::map);
    }
    ...
    public List<Xeste> siyahi(Long klinika, Boolean aktiv, String q, String xesteKodu, Long vesiqeNovuId, Long teskilatId, Long cinsId, java.time.LocalDate dogumBaslangic, java.time.LocalDate dogumBitme, Long olkeId, Long seherId, Long cursor, int limit) {
        String sql = "SELECT * FROM public.fn_xeste_siyahisi(p_klinika_id=>CAST(:k AS bigint),p_aktiv=>CAST(:a AS boolean)) WHERE (CAST(:q AS varchar) IS NULL OR lower(coalesce(xeste_kodu, '') || ' ' || coalesce(fin_kodu, '') || ' ' || coalesce(sexsiyyet_vesiqesi_nomresi, '') || ' ' || coalesce(ad, '') || ' ' || coalesce(soyad, '') || ' ' || coalesce(ata_adi, '') || ' ' || coalesce(mobil_nomre, '') || ' ' || coalesce(ikinci_mobil_nomre, '') || ' ' || coalesce(sosial_kart_nomresi, '') || ' ' || coalesce(is_yeri, '') || ' ' || coalesce(vezifesi, '') || ' ' || coalesce(pesesi, '')) LIKE '%' || lower(CAST(:q AS varchar)) || '%') AND (CAST(:kod AS varchar) IS NULL OR lower(xeste_kodu) LIKE lower(CAST(:kod AS varchar))||'%') AND (CAST(:vn AS bigint) IS NULL OR sexsiyyet_vesiqesi_novu_id=:vn) AND (CAST(:t AS bigint) IS NULL OR default_teskilat_id=:t) AND (CAST(:c AS bigint) IS NULL OR cins_id=:c) AND (CAST(:db AS date) IS NULL OR dogum_tarixi>=:db) AND (CAST(:ds AS date) IS NULL OR dogum_tarixi<=:ds) AND (CAST(:o AS bigint) IS NULL OR olke_id=:o) AND (CAST(:s AS bigint) IS NULL OR seher_id=:s) AND (CAST(:cursor AS bigint) IS NULL OR xeste_id<:cursor) ORDER BY xeste_id DESC LIMIT :limit";
    ...
    public long siyahiSayi(Long klinika, Boolean aktiv, String q, String xesteKodu, Long vesiqeNovuId, Long teskilatId, Long cinsId, java.time.LocalDate dogumBaslangic, java.time.LocalDate dogumBitme, Long olkeId, Long seherId) {
        String sql = "SELECT count(*) FROM public.fn_xeste_siyahisi(p_klinika_id=>CAST(:k AS bigint),p_aktiv=>CAST(:a AS boolean)) WHERE (CAST(:q AS varchar) IS NULL OR lower(coalesce(xeste_kodu, '') || ' ' || coalesce(fin_kodu, '') || ' ' || coalesce(sexsiyyet_vesiqesi_nomresi, '') || ' ' || coalesce(ad, '') || ' ' || coalesce(soyad, '') || ' ' || coalesce(ata_adi, '') || ' ' || coalesce(mobil_nomre, '') || ' ' || coalesce(ikinci_mobil_nomre, '') || ' ' || coalesce(sosial_kart_nomresi, '') || ' ' || coalesce(is_yeri, '') || ' ' || coalesce(vezifesi, '') || ' ' || coalesce(pesesi, '')) LIKE '%' || lower(CAST(:q AS varchar)) || '%') AND (CAST(:kod AS varchar) IS NULL OR lower(xeste_kodu) LIKE lower(CAST(:kod AS varchar))||'%') AND (CAST(:vn AS bigint) IS NULL OR sexsiyyet_vesiqesi_novu_id=:vn) AND (CAST(:t AS bigint) IS NULL OR default_teskilat_id=:t) AND (CAST(:c AS bigint) IS NULL OR cins_id=:c) AND (CAST(:db AS date) IS NULL OR dogum_tarixi>=:db) AND (CAST(:ds AS date) IS NULL OR dogum_tarixi<=:ds) AND (CAST(:o AS bigint) IS NULL OR olke_id=:o) AND (CAST(:s AS bigint) IS NULL OR seher_id=:s)";
    ...
    public Xeste tap(Long klinika, Long id) {
        return jdbc.query("SELECT * FROM public.fn_xeste_siyahisi(p_klinika_id=>CAST(:k AS bigint),p_aktiv=>NULL) WHERE xeste_id=:id", new MapSqlParameterSource("k", klinika).addValue("id", id), this::map).stream().findFirst().orElseThrow();
    }
