...
@Repository public class VezifeRepository{
 ...
 public List<Vezife> findAll(){return findAll(null);}
 public List<Vezife> findAll(Boolean aktiv){return jdbc.query("SELECT * FROM public.fn_vezife_siyahisi(CAST(:aktiv AS boolean)) ORDER BY sira_no NULLS LAST,vezife_adi",new MapSqlParameterSource("aktiv",aktiv),(r,n)->new Vezife(num(r.getObject("vezife_id")),null,null,r.getString("vezife_kodu"),r.getString("vezife_adi"),r.getString("aciqlama"),integer(r.getObject("sira_no")),r.getObject("aktiv",Boolean.class),r.getObject("yaranma_tarixi",java.time.LocalDateTime.class)));}
 ...
 public void update(Long id,String ad,String aciqlama,boolean aktiv){MapSqlParameterSource params=new MapSqlParameterSource().addValue("id",id).addValue("ad",ad.trim()).addValue("aciqlama",blank(aciqlama)).addValue("aktiv",aktiv);jdbc.queryForList("SELECT * FROM public.fn_vezife_yenile(p_vezife_id=>:id,p_ad=>:ad,p_aciqlama=>:aciqlama,p_aciqlama_deyisdirilsin=>true,p_aktiv=>:aktiv)",params);}
